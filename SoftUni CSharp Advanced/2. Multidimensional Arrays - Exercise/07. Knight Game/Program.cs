...
namespace _07._Knight_Game
{
    class Program
    {
        ...
        {
            if (IsInside(chessboard, row - 1, column + 2) && chessboard[row - 1, column + 2] == 'K')
            {
                currentEnemiesCount++;
            }
            if (IsInside(chessboard, row - 1, column - 2) && chessboard[row - 1, column - 2] == 'K')
            ...
            if (IsInside(chessboard, row - 2, column + 1) && chessboard[row - 2, column + 1] == 'K')
            {
                currentEnemiesCount++;
            }
            if (IsInside(chessboard, row - 2, column - 1) && chessboard[row - 2, column - 1] == 'K')
        ...
        {
            return row >= 0 && row < chessboard.GetLength(0) && column >= 0 && column < chessboard.GetLength(1);
        }
    }
}
